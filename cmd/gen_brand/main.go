...
func markGroup(x, y, size int) string {
	...
	for r := range composeCells {
		for c := range composeCells {
			...
			out = append(out, fmt.Sprintf(
				`    <rect x=%q y=%q width=%q height=%q rx=%q opacity=%q/>`,
				fixed(pad+float64(c)*step), fixed(pad+float64(r)*step),
...
`,
		...
		textX, repr(float64(top)+float64(t.markSize)*0.46), font, t.titleSize, heading, brandName,
		textX, repr(float64(top)+float64(t.markSize)*0.78), font, t.tagSize, body, tagline,
	), nil
}
