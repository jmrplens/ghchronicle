...
Ways to install ghchronicle: a page per operating system, five ways to get the binary running, and what each one is good for.
...
Running ghchronicle in Docker: the distroless image, what has to be mounted writable, and a compose file next to InfluxDB.
