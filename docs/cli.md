...
The ghchronicle command line: the flags, what each one does, and which of them print something and exit.
...
| `-card-width`     | the layout's  | Card width in pixels. Each layout draws between two ends of its own, which its [section](https://jmrp.io/docs/ghchronicle/card/layouts/) and `-card-layouts` both state; `badge-row` ignores it, its width following its pills |
