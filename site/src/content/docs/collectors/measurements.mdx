---
title: Measurements
description: Every measurement, its tags, its fields, and how each one is dated.
compactTables:
  - Dating
---
...
repository twice. They are one shape now, and `org` is gone because it was
`owner` with the personal half left blank.

This is a breaking change to what is already stored. Rows written by an earlier
release keep the old tags, new rows carry the new ones, and the two do not join:
in InfluxDB the tag set is part of a point's identity, so an upgraded collector
opens a second series for every repository rather than continuing the first.
Nothing is lost and nothing is rewritten, but a panel spanning the upgrade sees
two half-populated series. Recreating the database, or accepting the seam and
letting the old rows age out of the range you query, are the two ways through
it; there is no migration, because a tag cannot be renamed in place.
...
<Aside type="caution" title="The workflow tag changed meaning">
...
<Aside type="note" title="Why the weekly row is anchored to Sunday">
