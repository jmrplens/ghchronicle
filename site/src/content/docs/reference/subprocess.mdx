---
title: Calling it from a program
description: There is no Go library. What there is instead is one sweep, NDJSON on standard output, and good reasons not to call it in a loop.
searchTitle: Calling ghchronicle from a program
compactTables:
  - Key
  - Code
---
...
<Aside type="caution" title="There is no Go library">
...
<Aside type="tip" title="A default of 0 never switches the off ones on">
...
GraphQL point per repository for commits and one to two for issues. [Cost of a sweep](/ghchronicle/api/cost/) has the measured
...
<Aside type="note" title="When it is not ad hoc">
  If the program wants a continuous feed rather than an answer now, stop
  spawning it: run it as a service and give it a [sink](/ghchronicle/sinks/).
