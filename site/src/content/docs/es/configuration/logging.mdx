...
| ----------- | ----------- | ----------------------------------------------------- |
| `level`     | `info`      | `debug`, `info`, `warn` o `error`                     |
| `format`    | `text`      | `text` para una persona, `json` para un agente de envío    |
| `file`      | ninguno     | Un fichero rotatorio **además de** la salida de error |
| `max_bytes` | `67108864`  | Rota a los 64 MiB                                     |
| `keep`      | `5`         | Cuántos ficheros rotados conservar                    |
