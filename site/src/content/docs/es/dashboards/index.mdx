...
Dos de los cinco sinks saben describir su propio datasource, porque aquello a
lo que escriben es lo que Grafana consulta: InfluxDB y Elasticsearch. Los otros
tres no pueden, y ninguna lectura de sus ajustes lo encontraría. El sink de
Prometheus se raspa en vez de recibir escrituras, el sink SQL escribe
sentencias a un fichero en vez de a un servidor, y el de Graphite habla el
puerto de ingesta, que no es la API a la que va una consulta. Para esos, crea
el datasource en Grafana y nómbralo:
