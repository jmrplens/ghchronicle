...
`check_dashboards` informa de cada panel como correcto, vacío o fallido, y
cuenta aparte dos clases de fallo: un panel que nombra una tabla que el almacén
no ha creado, que es una familia que allí aún no se ha recogido y se llenará
sola, y un panel que nombra una columna que no ha creado, que es un panel que
nadie con el historial de esa cuenta puede dibujar. Lo segundo es la razón de
ejecutarlo contra un almacén con una cuenta real y no solo contra el banco de
pruebas: una columna de estos almacenes existe desde que un punto la ha
