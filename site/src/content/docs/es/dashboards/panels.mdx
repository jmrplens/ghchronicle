...
searchTitle: Qué muestran los dashboards de métricas de GitHub
---
...
estado actual y una suma sobre el rango contaría cada pasada. Se leen de
`gh_repo_total`, y con All seleccionado eso incluye los repositorios archivados
que el filtro por omisión aparta, que el selector no ofrece: nadie recorre su
historia, pero la gente sigue dándoles estrellas y haciéndoles fork, y cada
pasada de `totals` vuelve a leer sus contadores. Elegir repositorios cuenta
solo esos.
...
Lee `gh_account`, `gh_repo_total`, `gh_traffic` y `gh_contributions_total`.
...
commits totales, issues abiertas, lo fusionado en repositorios ajenos y en
cuántos hilos se comentó allí.
...
La última tarjeta cuenta hilos y no comentarios, cada issue o pull request una
vez por muchos comentarios que dejara la cuenta en ella, y solo en repositorios
que no son suyos, el mismo fuera que la tarjeta de al lado. La captura de
arriba es anterior a ambas cosas: su tarjeta pone "Comments elsewhere", y sus
96 incluían hilos de los repositorios propios de la cuenta.
...
actuales, aunque el selector no los ofrezca: la misma fila de cada uno se
vuelve a leer en cada pasada de `totals`.
...
Los forks en el tiempo son también un recuento acumulado, desde `gh_fork`, cada
fork con la fecha en que se hizo, así que también se remontan más allá del día
en que arrancó el colector en vez de empezar en la primera instantánea. Las dos
...
últimos trescientos eventos de los últimos treinta días y descarta las
notificaciones a los tres meses salvo las guardadas, así que lo que queda
almacenado es lo que había cuando pasó la pasada.
