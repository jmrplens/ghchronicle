---
title: Medidas
description: Cada medida que ghchronicle escribe a partir de la API de GitHub, sus etiquetas, sus campos y cómo se fecha cada una.
searchTitle: "Medidas de GitHub: etiquetas, campos y fechas"
compactTables:
  - Fechado
---
...
Noventa y tres medidas. Cada fila dice cómo se fecha un punto, porque eso es
...
Noventa y tres, y cada enlace cae en la tabla en la que está.
...
[`gh_code_scanning_setup`](#seguridad) ·
[`gh_collector_family`](#configuración-y-entrega) ·
[`gh_commit`](#desarrollo) ·
...
[`gh_star`](#estrellas-y-forks) · [`gh_star_day`](#estrellas-y-forks) ·
[`gh_star_given`](#estrellas-y-forks) · [`gh_star_list`](#cuenta) ·
[`gh_traffic`](#audiencia) ·
...
| `gh_star_day` | fechado, un punto por día: el día del Pacífico, a las 00:00 UTC | | `stars` |
...
[historial de estrellas](https://docs.github.com/es/rest/activity/starring#get-repository-star-history)
...
lee los cien primeros de cada uno, que contienen lo que se fusionó o se cerró
desde la pasada anterior por mucho tiempo que haga que se abrió, y un relleno
sigue leyendo hasta que se acaban las páginas o llegan a `backfill.since`.
...
| `gh_collector_family`   | ahora                  | `family`, `scope` (family, repo), `reason`      | `repos`, `failed`, `points`, `error`                                                                                                                                                                                                                                                                                                                     |
...
`gh_repo_total` es la tabla de la que se leen los totales de estrellas y forks
de la cuenta, en el Overview y en _Every repository, ever_, y no `gh_repo`. Las
dos difieren en un tipo de repositorio. El que el filtro por omisión aparta por
estar archivado no tiene fila en `gh_repo`, porque ninguna familia lo recorre,
pero sigue recibiendo y perdiendo estrellas y forks, así que la familia
`totals` escribe su `gh_repo_total` en cada pasada desde la consulta que fecha
su archivado: las mismas etiquetas y campos que la fila de un repositorio
recogido, `archived` a true, sellada en la pasada. Hasta la 2.5.1 solo un
relleno histórico la escribía, una vez, y el 2026-09-26 una de esas filas decía
4 estrellas donde GitHub decía 3. No recibe `gh_repo_policy`. Esa consulta
...
y una fila más por cada repositorio que perdió, nombrándolo como lo nombra
...
Las filas que siempre llegan son el motivo de todo esto. Una familia sin
ninguna fila en un barrido no se ejecutó en ese barrido, que es justo lo que un
panel vacío nunca podría decir, y son además lo que hace que la medida exista
en una cuenta donde nunca ha fallado nada: una tabla en la que InfluxDB no ha
escrito nunca no se dibuja vacía, se rechaza.
...
petición, y la cola es donde un fallo se explica. GitHub borra los logs pasado
el periodo de retención del repositorio, noventa días por omisión, y responde
410 cuando ya no están; un relleno solo pide los últimos noventa días, diga lo
que diga el ajuste, y como mucho 500 jobs fallidos por repositorio.
