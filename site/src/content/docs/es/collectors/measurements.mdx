...
| `gh_artifact_total`      | ahora                |                                                                           | `live_bytes`, `live_count`, `count`, `walked`, `complete`                                                                                               |
...
`gh_artifact_total` lleva cuatro cuentas porque su tamaño no está sobre
...
cuántos son, que es la cuenta sobre la que está ese tamaño. `complete` es
`walked >= count` en un solo valor: falso significa que las cifras vivas son un
suelo y no un total, que en ese repositorio se quedaba corto por un factor de
cincuenta y seis.
...
Lo raro que es, medido en la cuenta sobre la que se desarrolló esto: cuatro
filas en todo el almacén, ninguna en los últimos quince días, frente a 706 pull
requests de 829 que sí tuvieron una primera revisión de un bot o de su propio
autor. El campo no está roto; es la respuesta a una pregunta más estrecha de lo
que espera quien lee, que es por lo que el panel se llama por esa pregunta.
