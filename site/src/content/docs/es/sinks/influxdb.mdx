...
`points` es lo que se escribió, `unchanged` lo que el registro de escrituras ya
había enviado, y `filtered` lo que este destino descartó: una medida nombrada
aquí, y cualquier punto que no lleve ningún campo que el protocolo de línea
pueda representar. La clave solo aparece cuando hay algo que contar, y el total
de la pasada se imprime una vez al final. Hasta que se contó, la línea decía
`points=440` de una medida de la que la base de datos no ha tenido nunca una
fila.
