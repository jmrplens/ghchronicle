...
<Tabs syncKey="card-theme">
  <TabItem label="auto">
    Ambas paletas en un fichero, tras una media query `prefers-color-scheme`,
    que GitHub respeta. Un solo fichero funciona en un README con cualquier
    ajuste.
  </TabItem>
  <TabItem label="dark y light">
    Una paleta cada uno. Para un README que cambia con `#gh-dark-mode-only` y
    `#gh-light-mode-only`, dibuja la tarjeta dos veces: desde la Action son dos
    llamadas, una con `card-theme: dark` y otra con `card-theme: light`.
  </TabItem>
</Tabs>
