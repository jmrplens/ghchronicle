...
searchTitle: Por qué cada punto lleva la fecha en que ocurrió
compactTables:
  - Tipo
  - Regla
---
...
### Las once que no se sirven nunca

Once de las medidas llevan `skip`, así que un exportador de Prometheus y un
backend OTLP con `raw: false` no las ven nunca. Ocho son historia, dos son
...
| `gh_star_day`              | Historia. Estrellas por día, corregidas hacia atrás cuando se quita una estrella dada en las últimas treinta semanas; el recuento es `gh_repo.stars` |
...
segunda copia del año. `gh_star_day` se ancla igual, cada día la `week` que
devuelve GitHub más su índice. El día es el día natural del propio GitHub en
America/Los_Angeles, medido y no documentado, y la fila se sella a las 00:00 UTC
de esa fecha, así que una estrella dada una mañana europea puede quedar un día
antes del instante que le da `gh_star`.
