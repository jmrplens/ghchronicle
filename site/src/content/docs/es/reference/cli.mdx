...
| `-backfill-status` | apagada      | Imprime hasta dónde ha llegado el relleno en curso, y sale; no pregunta nada a GitHub, no escribe nada y no necesita token                 |
| `-backfill-retry` | `0`           | Cuando un relleno acaba con familias pendientes, espera esto y vuelve a por ellas, hasta que una pasada no anote nada nuevo; `0` no vuelve  |
| `-publish-dashboard` | off        | Publica el dashboard de Grafana y el datasource del que lee, y sale; necesita la sección `grafana`, no le pregunta nada a GitHub y no necesita token |
...
| `-yes`            | off           | Sigue adelante con `-uninstall` en vez de solo listar lo que quitaría                                                                      |
