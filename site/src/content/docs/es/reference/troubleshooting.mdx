...
**El almacenamiento de artefactos parece pequeño.** Mira `complete` en
`gh_artifact_total`, o `walked` frente a `count`. Un `complete` falso significa
que el tamaño vivo es un suelo: el repositorio tiene más artefactos de los que
recorrió el tope de páginas. También puede salir más pequeño de lo esperado por
una segunda razón, que `complete` no cubre: `count` es el total de GitHub e
incluye los artefactos que ya ha caducado, mientras que el tamaño es solo sobre
los vivos, que son los que cuenta `live_count`.
