...
| ----------------- | ------------------------ | -------------------------------------------------------------------------------------------------------- |
| `token`           | obligatoria              | Un token personal. El `GITHUB_TOKEN` automático no basta para la mayor parte de lo que se recoge         |
...
1. Crea un token personal (ver [el token](/ghchronicle/es/start/token/)) y
   guárdalo en `<tú>/<tú>` como el secreto `GHCHRONICLE_TOKEN`. Para una tarjeta
   de números públicos basta el `GITHUB_TOKEN` automático, y entonces no hace
   falta ningún secreto: pasa `${{ secrets.GITHUB_TOKEN }}` en su lugar.
...
**Un README que no está en la raíz.** Las rutas del `<picture>` son relativas al
README, así que un `docs/README.md` apunta a `../generated/card.svg`.

<Aside type="caution" title="Repositorios privados">
  Sin fichero de configuración, la Action cuenta solo repositorios públicos. Con
  `include-private: true` la tarjeta suma las estrellas, forks, lenguajes y
  tráfico de tus repositorios privados, y los layouts que listan repositorios
  (`repo-list`, y cualquier layout al que se le pida el campo `top_repos`)
  **publican sus nombres**. Para contarlos sin nombrarlos, elige tú los campos y
