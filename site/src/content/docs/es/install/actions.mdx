...
description: "ghchronicle como GitHub Action: la Action compuesta, sus tres modos y las dos cosas que un runner alojado no conserva."
searchTitle: ghchronicle como GitHub Action
defaultColumns:
  - table: Entrada
    default: Por omisión
---
...
| `include-private` | `false`                  | `true` cuenta los repositorios privados cuando no se da fichero de configuración. Ver el aviso más abajo |
| `version`         | `latest`                 | La release que instalar: `latest` para la más nueva, o una release con o sin su `v`, así que `__VERSION__` y `v__VERSION__` son la misma. La etiqueta mayor `v2` es lo que toma `uses:`, no una release, y se rechaza |
...
<Aside type="caution" title="Repositorios privados">
  ...
  de la cuenta, tal como GitHub los muestra en tu perfil. Con
...
<Aside type="caution" title="El token tiene que ser un token personal">
  ...
  en un token fine-grained, el permiso de repositorio Administration (lectura);
  el `GITHUB_TOKEN` automático no puede recibir Administration, así que recibe
  un 403 incluso en el repositorio en el que corre el workflow. Tampoco tiene
  `security_events` ni `read:packages`, y no es un usuario, así que nada de
  ámbito de cuenta funciona con él. Ver [el token](/ghchronicle/es/start/token/).
</Aside>

**El fichero de estado no sobrevive entre ejecuciones.** Cada ejecución vuelve
por tanto a recorrer entera la lista de estrellas, y lee entero el historial
diario de estrellas de cada repositorio, una página por cada treinta semanas de
su vida. En una cuenta pequeña son un puñado de llamadas; en una cuenta con
muchas estrellas o con repositorios antiguos, guárdalo en caché:
...
saltarse los dos recorridos, y nunca lo vuelve a escribir: entrega sus puntos a la
