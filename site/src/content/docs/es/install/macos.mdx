...
<Aside type="caution" title="Si macOS se niega a ejecutarlo">
  ...
  abrir porque no se puede verificar el desarrollador». `curl` no pone ese
  atributo y un navegador sí, que es por lo que la descarga de arriba es un
  `curl`. Si la tuya vino de un navegador, quítalo y comprueba que ya no está:
...
Go 1.27 o más nuevo es lo que declara el módulo. La compilación fija
