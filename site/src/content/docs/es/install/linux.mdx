...
`/usr/local/bin` si tiene permiso de escritura y en `~/.local/bin` si no. Una
suma que no cuadra lo detiene sin instalar nada, y no hay opción para saltarse
ese paso. Si `cosign` ya está en la máquina, verifica además que el propio
fichero de sumas viene del workflow de release.
