...
| | En Windows |
| --- | --- |
| El programa | `ghchronicle.exe`. Escribir `ghchronicle` lo encuentra, porque `PATHEXT` lista `.EXE` |
| Dependencias en ejecución | Ninguna. El binario se compila con `CGO_ENABLED=0`, así que no hay runtime de C que instalar |
| Pararlo | Ctrl+C en su consola. No hay señal que enviarle, y `taskkill /F` lo termina sin cerrar sus destinos |
...
<Aside type="caution" title="Nombra la versión en vez de pedir la más nueva">
  ...
  necesita una. La release `v1` **no lleva ningún fichero**, así que una URL de
  descarga construida a partir de la release que GitHub marque en ese momento
  puede acabar en una release vacía. Coge la versión de la
...
<Aside type="caution" title="Lee el Path del mismo sitio en el que lo escribes">
...
Windows PowerShell 5.1, el que viene de serie, escribe UTF-16 con `>` y con
`Set-Content` salvo que se le diga otra cosa. Un analizador de YAML lee eso
como binario. PowerShell 7 usa UTF-8 sin marca de orden de bytes por omisión y
no tiene ese problema; en 5.1, sé explícito:
...
<Tabs syncKey="windows-task">
  <TabItem label="Una pasada con temporizador">
    ...
    Una tarea horaria da a todas las familias una cadencia horaria como mucho,
    así que el ritmo de quince minutos de `actions` se pierde. Es el mismo
    cambio que
  ...
  <TabItem label="Corriendo todo el rato">
    ...
    omisión para una tarea a los tres días, que para un proceso pensado para no
    terminar es un reinicio que nadie pidió.
...
`Stop-ScheduledTask` termina el proceso en vez de pedirle que acabe, que es el
`taskkill /F` de la primera tabla con otro nombre. En la forma con
temporizador no hay nada que parar, que es buena parte de por qué es la mejor
opción por omisión.
...
Go 1.27 o más nuevo. `CGO_ENABLED=0` quiere decir sin MSVC, sin MinGW y sin SDK
de Windows.
