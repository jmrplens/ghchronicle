...
<Card name="terminal" animated loop alt="The terminal layout played once: a terminal window whose lines of output each have a number typing itself in, with a block cursor blinking at the prompt below them, and the page can also play it in a loop, where the typing still happens once and only the cursor goes on blinking" />
