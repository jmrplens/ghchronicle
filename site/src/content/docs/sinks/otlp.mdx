---
title: OpenTelemetry
description: "Sending GitHub metrics to an OpenTelemetry collector or to Prometheus: OTLP over HTTP with the JSON encoding, and why raw is false by default."
searchTitle: Send GitHub metrics over OpenTelemetry (OTLP)
---

import { Aside, Tabs, TabItem } from "@astrojs/starlight/components";

The OTLP sink pushes ghchronicle's metrics to an OpenTelemetry collector, or straight into Prometheus through its OTLP receiver.
...
dependencies out of the binary. Every sink but PostgreSQL, which needs its
driver, speaks its protocol with the standard library alone.
...
<Tabs syncKey="otlp-raw">
  <TabItem label="raw: false (default)">
    ...
    [the same reduction](/ghchronicle/sinks/prometheus/#the-reduction), so each
