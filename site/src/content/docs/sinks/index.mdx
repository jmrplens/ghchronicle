...
description: "Where to store GitHub metrics: eleven stores, what each one can and cannot answer, and the one property that decides between them."
searchTitle: Where to store GitHub metrics
compactTables:
  - Store
---
...
| [PostgreSQL](/ghchronicle/sinks/postgres/)         | the dated history, in a database it connects to | a Grafana user with a Postgres and no InfluxDB | `dsn`, `batch`                  |
| [PostgreSQL, to a file](/ghchronicle/sinks/postgres/) | the same tables, as SQL you pipe into psql | a load that happens later, or elsewhere        | `dialect`, `path`               |
