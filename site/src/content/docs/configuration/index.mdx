---
title: The file
description: "The ghchronicle configuration: one YAML file, every value expandable from the environment, and what each top-level block is for."
searchTitle: The ghchronicle configuration file
defaultColumns:
  - table: Key
    default: Default
compactTables:
...
| `base_url`     | api.github.com       | A GitHub Enterprise instance uses `https://<host>/api/v3`. GitHub Enterprise Server does not serve the daily star history, so there [`gh_star_day`](/ghchronicle/collectors/measurements/#stars-and-forks) and the star-count panels drawn from it stay empty |
...
again](/ghchronicle/how/backfill/#stopping-one-and-picking-it-up-again).
