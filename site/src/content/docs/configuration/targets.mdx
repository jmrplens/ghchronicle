...
searchTitle: Which GitHub repositories a sweep covers
defaultColumns:
  - table: Key
    default: Default
---
...
- **An archived repository takes no more work.** Its stars, forks and
  watchers can still move, and those are read anyway, as below. Nothing else
  can, and collecting it spends the budget on rows that never move again.
...
of them in one query per twenty five at its own cadence, so the _Repositories
archived_ table exists from the first sweep, the account's star and fork
totals count them, and the cost is a point per twenty five archived
repositories per `totals` sweep. And a [backfill](/ghchronicle/how/backfill/) collects archived
