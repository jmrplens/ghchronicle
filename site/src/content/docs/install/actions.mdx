...
| ----------------- | -------------------- | -------------------------------------------------------------------------------------------------- |
| `token`           | required             | A personal access token. The automatic `GITHUB_TOKEN` is not enough for most of what this collects |
...
1. Create a personal access token (see [the token](/ghchronicle/start/token/)) and
   store it in `<you>/<you>` as the secret `GHCHRONICLE_TOKEN`. For a card of
   public numbers the automatic `GITHUB_TOKEN` is enough, and then no secret is
   needed: pass `${{ secrets.GITHUB_TOKEN }}` instead.
...
**A README that is not at the root.** The paths in the `<picture>` are relative
to the README, so a `docs/README.md` points at `../generated/card.svg`.

<Aside type="caution" title="Private repositories">
  Without a config file, the Action counts public repositories only. With
  `include-private: true` the card adds the stars, forks, languages and traffic
  of your private repositories, and the layouts that list repositories
  (`repo-list`, and any layout given the `top_repos` field) **publish their
  names**. To count them without naming them, pick the fields yourself and leave
