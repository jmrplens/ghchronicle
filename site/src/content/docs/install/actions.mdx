...
a sweep of its own. That holds as written when the Action writes its own
configuration, because each use then gets a state file of its own. With a
`config` file, give each step its own file with a different `state_file`: a
second sweep that reads the first one's state is answered "not modified" for
everything and draws a card of zeros.
