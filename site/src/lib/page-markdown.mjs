...
function renderSelfClosing(name, attributes, raw, context) {
	...
	// A picker is a picker, and a reader of text cannot pick. What it offers
	// does reduce: every compose file it can show, each under the stack it
	// belongs to, which is what somebody reading the markdown twin or handing
	// it to a model actually needs.
	if (name === "ComposePicker") {
		return `\n\n${composeMarkdown()}\n\n`;
	}
	if (name === "LinkCard") {
...
/**
 * Every generated compose file, as fenced blocks under a heading each.
 *
 * Read from `deploy/` rather than restated, for the reason the picker itself
 * reads them: the files are generated by `cmd/gen_compose` and brought up by
 * the end-to-end suite, so a copy here would be the one thing nobody tested.
 *
 * @returns {string}
 */
function composeMarkdown() {
	...
	const titles = {
		"compose.influxdb-grafana.yaml": "InfluxDB and Grafana",
		"compose.influxdb.yaml": "InfluxDB",
		"compose.postgres-grafana.yaml": "PostgreSQL and Grafana",
		"compose.postgres.yaml": "PostgreSQL",
		"compose.collector.yaml": "The collector on its own",
	};
